...
int CameraHandler(void) {
   ...
   /**************************** 新增显示代码 ****************************/
    if (readFlag(showImg_file)) {
        ...
        // 使用原始帧
        cv::Mat processedFrame;
        cv::resize(raw_frame, processedFrame, cv::Size(newWidth, newHeight));
        
        // 在原始图像上绘制车道线标记
        const int lineThickness = 2;  // 线宽
        for (int y = 0; y < line_tracking_height; ++y) {
            // 计算缩放后坐标
            const int scaledY = y * calc_scale;
            const int scaledLeft = left_line_filtered[y] * calc_scale;
            const int scaledRight = right_line_filtered[y] * calc_scale;
            const int scaledMid = mid_line_filtered[y] * calc_scale;
            
            // 只绘制有效的点
            if (scaledLeft >= 0 && scaledLeft < newWidth) {
                cv::line(processedFrame, 
                         cv::Point(scaledLeft, scaledY), 
                         cv::Point(scaledLeft, scaledY),
                         cv::Scalar(0, 0, 255), lineThickness);  // 左线-红色
            }
            
            if (scaledRight >= 0 && scaledRight < newWidth) {
                cv::line(processedFrame, 
                         cv::Point(scaledRight, scaledY), 
                         cv::Point(scaledRight, scaledY),
                         cv::Scalar(0, 255, 0), lineThickness);   // 右线-绿色
            }
            
            if (scaledMid >= 0 && scaledMid < newWidth) {
                cv::line(processedFrame, 
                         cv::Point(scaledMid, scaledY), 
                         cv::Point(scaledMid, scaledY),
                         cv::Scalar(255, 0, 0), lineThickness);   // 中线-蓝色
            }
        }
